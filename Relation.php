    ...
    final class Relation extends \Components\Validation {
        public function __construct($table, array $tables, string $join = NULL, array $joins = []) {             
            foreach ($tables as $thatTable) {
                if ($thatTable instanceof \Components\Core\Mapper && in_array((string) $thatTable, $table->relations)) {              
            ...
            foreach ($tables as $thatTable) {
                if ($thatTable instanceof \Components\Core\Mapper) {              
                    foreach ($tables as $thisTable) {
                       if ($thisTable instanceof \Components\Core\Mapper && !array_key_exists((string) $thisTable, $joins) && isset($thisTable->relations) && in_array((string) $thatTable, $thisTable->relations) && isset($thisTable->keys) && in_array((string) $thatTable, $thisTable->keys)) {
