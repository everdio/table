...
namespace Modules\Table {
    final class Find extends \Components\Validation {
        public function __construct(array $validations = [], string $select = NULL, string $from = NULL, array $relations = [], array $operators = []) {                  
            ...
            parent::__construct($select . $from . implode(false, $relations) . (sizeof($operators) ? sprintf("WHERE%s", implode(false, $operators)) : false), [new \Components\Validator\IsString\Contains(["SELECT", "FROM"])]);
        }
    }
}
